(function () {
      ...
      if (barcodeReq.readyState === 4 && barcodeReq.status === 200) {
        ...
        if ("offset" in responseJSON) {
          ...
          listOfAttendees.forEach(function (participant, index, array) {
            console.log(
              "Processing barcode for participant",
              index + 1,
              "out of",
              listOfAttendees.length,
              "..."
  ...
  function getSuccessMessage(index) {
    var currentEventName =
      eventDropdown.options[eventDropdown.selectedIndex].textContent;
  ...
  function getEventList() {
    ...
    airtableGetRequest.onreadystatechange = function () {
      if (
        airtableGetRequest.readyState === 4 &&
        airtableGetRequest.status === 200
      ) {
